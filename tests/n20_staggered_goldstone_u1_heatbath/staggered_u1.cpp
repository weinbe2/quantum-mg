...
int main(int argc, char** argv)
{
  ...
  // Information about the staggered operator.
  double mass = 4e-2;

  // Define inverter parameters, inversion struct.
  inversion_verbose_struct* verb = new inversion_verbose_struct(VERB_SUMMARY, std::string("[QMG-STAGGERED-INFO]: "));
  int max_iter = 4000;
  double tol = 1e-10;
  int bicgstab_l = 6;
  ...
  // Create a first staggered operator. 
  Staggered2D* staggered = new Staggered2D(lat, mass, gauge_field);
  ...
  for (i = n_update; i < n_max; i+=n_update)
  {
    ...
    if (i > n_therm)
    {
      ...
      // Update the staggered operator.
      staggered->update_links(gauge_field);
      cout << setiosflags(ios::scientific) << setprecision(6);

      ////////////////
      src[lat->cv_coord_to_index(0,0,0)] = 1.0;
      invif = minv_vector_bicgstab_l(prop, src, cv_size, max_iter, tol, bicgstab_l, apply_stencil_2D_M, (void*)staggered, verb);
  ...
  for (j = 0; j < y_len; j++)
    cout << "[QMG-PION-CORR]:" << j << " " << pion[j]/count << " +/- " << sqrt((pion_sq[j]/count - pion[j]*pion[j]/(count*count))/(count)) << "\n";
  cout << "[QMG-END-PION]\n";
