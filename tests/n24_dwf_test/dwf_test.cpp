...
int main(int argc, char** argv)
{
  ...
  Dwf2D* dwf_stencil = new Dwf2D(lat, mass, gauge_field, Ls, M5);
  dwf_stencil->build_dagger_stencil();
  ...
  invif = minv_vector_bicgstab(lhs, rhs, cv_size, max_iter, tol, apply_stencil_2D_M, (void*)dwf_stencil, verb);
  // Check results.
  ...
  copy_vector(lhs, init_guess, cv_size);
  copy_vector(rhs, init_rhs, cv_size);
  verb = new inversion_verbose_struct(VERB_DETAIL, std::string("[QMG-TEST-BICGSTAB-") + to_string(Ls) + std::string("-INFO]: "));
  invif = minv_vector_bicgstab_l(lhs, rhs, cv_size, max_iter, tol, bicgstab_l, apply_stencil_2D_M, (void*)dwf_stencil, verb);
  // Check results.
  if (invif.success == true)
  {
    cout << "[QMG-TEST-BICGSTAB-" << Ls << "]: Algorithm " << invif.name << " took " << invif.iter
      << " iterations to reach a tolerance of "
      << sqrt(invif.resSq)/rhs_norm << "\n";
  }
  else // failed, maybe.
  {
    cout << "[QMG-TEST-BICGSTAB-" << Ls << "]: Potential Error! Algorithm " << invif.name
      << " took " << invif.iter << " iterations to reach a tolerance of "
      << sqrt(invif.resSq)/rhs_norm << "\n";
  }
  cout << "[QMG-TEST-BICGSTAB-" << Ls << "]: Computing [check] = A [lhs] as a confirmation.\n";
  // Check and make sure we get the right answer.
  zero_vector(check, cv_size);
  dwf_stencil->apply_M(check, lhs);
  explicit_resid = sqrt(diffnorm2sq(init_rhs, check, cv_size))/rhs_norm;
  cout << "[QMG-TEST-BICGSTAB-" << Ls << "]: The relative error is " << explicit_resid << "\n";
  delete verb;
  ...
  invif = minv_vector_tfqmr(lhs, rhs, cv_size, max_iter, tol, apply_stencil_2D_M, (void*)dwf_stencil, verb);
  // Check results.
  if (invif.success == true)
  {
  ...
  // Prepare
  zero_vector(check, cv_size);
  dwf_stencil->prepare_M_dagger_M(check, rhs);
  copy_vector(rhs, check, cv_size);
  // Done prepare
  invif = minv_vector_cg(lhs, rhs, cv_size, max_iter, tol, apply_stencil_2D_M_dagger_M, (void*)dwf_stencil, verb);
  // Check results.
  if (invif.success == true)
  {
