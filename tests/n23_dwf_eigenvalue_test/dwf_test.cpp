...
int main(int argc, char** argv)
{
  ...
  if (cv_size <= 2048)
  {
    ...
    arpack = new arpack_dcn(cv_size, max_iter, tol, apply_stencil_2D_M, (void*)dwf_stencil);
    ...
    for (int i = 0; i < cv_size; i++)
      std::cout << "Eigenvalue " << i << " has value " << real(eigs[i]) << " +/- I " << imag(eigs[i]) << "\n";
  ...
  arpack = new arpack_dcn(cv_size, max_iter, tol,
                apply_stencil_2D_M, (void*)dwf_stencil,
                n_eigens, 3*n_eigens);
